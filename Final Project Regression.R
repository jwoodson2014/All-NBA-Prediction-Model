...
model_df <- df_all_seasons %>%
  select(-c(slugSeason,namePlayer,yearSeason,
            ...
            unique)) %>%
  mutate(dummy_cols(model_df$groupPosition)) %>%
...
for (treesize in (5:25)) {
  ...
  df_parameters[nrow(df_parameters)+1,"treesize"] <- treesize
...
df_filter <- df_all_seasons %>%
  filter(isALLNBA==1) %>%
  group_by(yearSeason) %>%
  summarise(PPG = mean(ptsPerGame),
            RPG = mean(trbPerGame),
            APG = mean(astPerGame),
            TSP = mean(pctTrueShooting),
            VORP = mean(ratioVORP))

df_filter %>%
  ggplot(aes(x=yearSeason,y=VORP))+
  geom_line()

x <- pretty.gbm.tree(model_fit_1)
